...
- **G8 — Multi-frame in flight:** no `vkDeviceWaitIdle` in the steady loop; a concrete instrument — an "submitted-but-timeline-not-yet-signaled" outstanding-frame counter — reaches ≥2 before timeline value 1 is signaled, asserted via a grep-able log line (e.g. `[frames_in_flight] peak=2`). Pass condition: `peak >= MAX_FRAMES_IN_FLIGHT`.
